...
def teardown_test(utils):
    ...
    plugin_conf = os.path.join(plugin_conf_path, 'tdnfmetalinker.conf')
    #os.remove(plugin_conf)
    tdnf_repo = os.path.join(utils.tdnf_config.get('main', 'repodir'), 'photon-test.repo')
    #utils.run(['sed', '-i', '/tdnfmetalinker/d', tdnf_repo])
...
# make sure libtdnfmetalinker.so is loaded without issues
def test_tdnfmetalinker_plugin_load(utils):
    enable_plugins(utils)
    ret = utils.run(['tdnf', 'repolist'])
    # we should load the plugin
    #assert(ret['stdout'][0].startswith('Loaded plugin: tdnfmetalinker'))  # nosec
    print(ret) 
