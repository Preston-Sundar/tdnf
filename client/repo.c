...
uint32_t
TDNFEventRepoMDDownloadInit(
    PTDNF pTdnf,
    const char *pszRepoDataDir,
    char* pszRepoMetaLinkURL,
    PTDNF_PLUGIN_MD_FLAGS pStatusFlags
    )
{
    uint32_t dwError = 0;
    TDNF_EVENT_CONTEXT stContext = {0};

    if (!pTdnf || !pStatusFlags)
    {
        dwError = ERROR_TDNF_INVALID_PARAMETER;
        BAIL_ON_TDNF_ERROR(dwError);
    }
    ...
    stContext.nEvent = MAKE_PLUGIN_EVENT(
                           ...
                           TDNF_PLUGIN_EVENT_PHASE_INIT);

    dwError = TDNFAddEventDataString(
        &stContext,
    ...
    )
{
    ...
    stContext.nEvent = MAKE_PLUGIN_EVENT(
                           TDNF_PLUGIN_EVENT_TYPE_REPO_MD,
                           TDNF_PLUGIN_EVENT_STATE_REFRESH,
                           TDNF_PLUGIN_EVENT_PHASE_START);

    dwError = TDNFAddEventDataPtr(
        &stContext,
        TDNF_EVENT_ITEM_REPO_SOLV_COOKIE,
        pszCookie);
    BAIL_ON_TDNF_ERROR(dwError);
...
TDNFEventRepoMDDownloadStart(
    ...
    const char **ppcszRepoMDUrl,
    const char *pcszTmpRepoMDFile,
    const char *pcszRepoMDFile,
    const char *pcszTmpRepoDataDir
    )
{
    uint32_t dwError = 0;
    TDNF_EVENT_CONTEXT stContext = {0};
    ...
    )
{
    ...
    // note: <THESE FLAGS CAN BE SET BY PLUGINS, MOVED TO A STRUCT>
    // int nNeedDownload = 0; // FLAG 1
    // int nNewRepoMDFile = 0; // FLAG 2
    // int nReplaceRepoMD = 0; // FLAG 3 
    // int nReplacebaseURL = 0; // FLAG 4
    PTDNF_PLUGIN_MD_FLAGS pStatusFlags = NULL;
    ...
    /* TODO: This metalink str check needs to be inside the plugin. */
    ...
    // Allocate space for plugin status flags. 
    dwError = TDNFAllocateMemory(
                  1,
                  sizeof(TDNF_PLUGIN_MD_FLAGS),
                  (void **)&pStatusFlags);
    BAIL_ON_TDNF_ERROR(dwError);
    ...
    /* plugin event indicating a repomd download has started. */
    /* TODO: This pszMetaLink string should not be passed to the plugin */
    dwError = TDNFEventRepoMDDownloadInit(
        pTdnf,
        pszRepoDataDir,
    ...
    if (!pStatusFlags->nPluginHandlesDownload)
    {
    ...
    if (pTdnf->pArgs->nRefresh)
    {
        ...
        /* plugin event indicating a repomd refresh has started. */
        dwError = TDNFEventRepoMDRefreshStart(
            pTdnf,
            pszCookie);
        BAIL_ON_TDNF_ERROR(dwError);
        ...
        // else
        // {
        if (!pStatusFlags->nPluginHandlesRefresh)
        {
            if (!access(pszRepoMDFile, F_OK))
            {
    ...
    if (pStatusFlags->nNeedDownload && !pTdnf->pArgs->nCacheOnly)
    {
        ...
        /* plugin event indicating a repomd download started. */
    ...
    //     /******METALINK****** -- DONE */
        ...
        if (pStatusFlags->nNewRepoMDFile)
        {
            /* plugin event indicating a repomd download ended. */
...
cleanup:
    ...
    TDNF_SAFE_FREE_MEMORY(pStatusFlags);
    ...
    )
{
    ...
    /* coverity[toctou] */
    if (rename (pszSrcFile, pszDstFile) != 0)
    {
        dwError = errno;
        BAIL_ON_TDNF_SYSTEM_ERROR(dwError);
    }
